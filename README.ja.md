...
- Worker pipeline: `prepareImageInBrowserPipeline()` で worker 優先の準備、cancel、fallback を扱う
...
転送前に upload policy へ収めたい場合は、`prepareImageToBudget()` または worker 優先の `prepareImageInBrowserPipeline()` と明示的な upload adapter を足してください。upload wiring は app 側で所有します。
